/*
  ...
  removed identifying text from lora string
  changed temp sensor power output pin
  added raw adc value to lora and csv strings 
...
//aaa additions for turbidity pump motor driver
...
void loop () {
  ...
  while (battVoltUpdate() <= 3.3) { //Power protection set to work for primary and secondary cells
...
void buildDataStrings() {
  ...
  if (sensor.temp.sensorCount > 0) {
    for (int i = 0; i < sensor.temp.sensorCount; i++) {
      LoRaDataString +=  String(sensor.temp.measure[i], 2) + ",";   //"TO" + String(i) + "=" + was moved by RJ after "LoRaDataString +="
  ...
  if (sensor.PT100.sensorCount > 0) {
    LoRaDataString +=  String(sensor.PT100.measure[0], 3) + ",";     // "TP1=" +   was moved by RJ after "LoRaDataString +="
  ...
  for (int j = 0; j < 4 ; j++) {
    if (sensor.adc[j].sensorCount > 0) {
      ...
      LoRaDataString += String(sensor.adc[j].measure[0], 2) + "," + String(sensor.adc[j].measure_2[0], 2) + ",";      //"A_" + String(cfg.adc[j].id) + "=" +    was moved by RJ after "LoRaDataString +="
      // CSVDataString += String(sensor.adc[j].measure[0], 2) + ",";    //Raw
      CSVDataString += String(sensor.adc[j].measure[0], 2) + "," + String(sensor.adc[j].measure_2[0], 2) + ",";     //Calibrated
    }
  }
  CSVDataString += "\n";
  //aaa
...
bool ADCUpdate(int adcChannel) {
  ...
  sensor.adc[adcChannel].measure[0] = 0;   // Reset or clear any previous voltage reading for the first sample ([0]) of the selected ADC channel
  sensor.adc[adcChannel].measure[0] += ads.readADC(adcChannel) * f * 1000;  // Read raw ADC steps, convert to Volts (* f), convert to millivolts (* 1000), and add to the measurement slot

  // Loop to take the remaining number of samples defined in the configuration settings
  // (Starts at k = 1 because the very first sample index '0' was already taken outside this loop)
  for (int k = 1; k < cfg.adc[adcChannel].aveCount; k++) {

    // Wait for the configured stabilization delay between consecutive readings to prevent cross-talk or noise
    delay(cfg.adc[adcChannel].aveDelay);

     // Read the ADC, convert to millivolts, and accumulate (add) the result to the total sum
    sensor.adc[adcChannel].measure[0] += ads.readADC(adcChannel) * f * 1000;

  }

  // Divide the accumulated total by the sample count to get the final averaged voltage in millivolt
